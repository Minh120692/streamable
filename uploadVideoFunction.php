...
if ($_SERVER["REQUEST_METHOD"] === 'POST') {
    ...
    if (isset($_FILES['video']) && $_FILES['video']['error'] == 0) {
        ...
        $target_file = $target_dir_video . basename($file_name);
        ...
        if (!in_array($videoFileType, $allowedTypes)) {
            echo json_encode(['status' => 400, 'message' => 'Only MP4, AVI, MOV, and MPEG files are allowed.']);
            $uploadOk = 0;
        }

        // Kiểm tra nếu $uploadOk để xử lý tải lên
        if ($uploadOk == 1) {
            if (move_uploaded_file($videoFileTmp, $target_file)) {
                ...
                try {
                    $duration = $ffprobe->format($target_file)->get('duration');
                } catch (Exception $e) {
                    echo json_encode(['status' => 500, 'message' => 'Error getting video duration: ' . $e->getMessage()]);
                    exit;
                }

                // Lấy độ phân giải (resolution)
                $video_stream = $ffprobe->streams($target_file)->videos()->first();
                ...
                $thumbnail_file = $thumbnail_dir . 'thumb_' . basename($file_name) . '.jpg';
                $thumbnail = 'thumb_' . basename($file_name) . '.jpg';
                ...
                if ($stmt->execute()) {
                    ...
                    echo json_encode(['status' => 200, 'message' => 'Video uploaded and information saved successfully.']);
                } else {
                    echo json_encode(['status' => 500, 'message' => 'Error: ' . $stmt->error]);
                }
                $stmt->close();
            } else {
                echo json_encode(['status' => 500, 'message' => 'Sorry, there was an error uploading your video: ' . basename($file_name)]);
            }
        }
    } else {
